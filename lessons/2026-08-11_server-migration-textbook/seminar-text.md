...
**この節が、2章でいちばん大事なところです。**

似ているけれど、まったく違う3つがあります。

| | 何が変わるか | サーバー移転で使うか |
|---|---|---|
| **ドメイン移管** | 管理会社（レジストラ） | 使わない |
| **ネームサーバー変更** | DNSの置き場 | **これを使う** |
| **所有者変更**（Whois名義変更） | ドメインの持ち主 | 使わない |

序章の「表札」のたとえで言うと、こうなります。

- **ドメイン移管** … 表札を作ってもらう業者を変える
- **ネームサーバー変更** … 表札に書いてある住所を書き換える
- **所有者変更** … 家の名義を変える

> **ポイント**
> サーバー移転でやるのは、**ネームサーバー変更**だけです。
> ドメイン移管も所有者変更も、サーバー移転には必要ありません。

この3つがごちゃ混ぜのまま作業を始めると、

- 必要のない移管をしてしまう
- 移管中にネームサーバーも触ってしまい、二重に事故を起こす

ということが起きます。

<!-- 図の差し込み位置：「ドメイン移管／ネームサーバー変更／所有者変更」の3点比較図 -->
